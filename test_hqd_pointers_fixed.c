...
void check_hqd_pointers(const char* when) {
    ...
    fflush(stdout);
    system("umr -r *.*.CP_HQD_PQ_WPTR 2>/dev/null | grep -o '0x[0-9a-fA-F]*' || echo 'N/A'");
    printf("  CP_HQD_PQ_RPTR: ");
    fflush(stdout);
    system("umr -r *.*.CP_HQD_PQ_RPTR 2>/dev/null | grep -o '0x[0-9a-fA-F]*' || echo 'N/A'");
}
...
int main(int argc, char **argv) {
    ...
    // 6. Create BO list - different methods for different kernel versions
    uint32_t num_handles = use_scratch ? 1 : 2;
    uint32_t bo_list_handle = 0;
    
    // Method 1: Try with drm_amdgpu_bo_list_entry array
    ...
    if (ret < 0) {
        // Method 2: Try with simple handle array
    ...
    // 7. Build CS with TWO chunks (or one for scratch test)
    int num_chunks = use_scratch ? 1 : 2;
    ...
    // BO_HANDLES chunk (if using BO list)
    if (!use_scratch) {
        uint32_t handles[2] = {ib_handle, signal_handle};
    ...
    cs_args.in.num_chunks = num_chunks;
    ...
    if (use_scratch) {
        printf("  CP_SCRATCH0: ");
        fflush(stdout);
        system("umr -r *.*.CP_SCRATCH0 2>/dev/null | grep -o '0x[0-9a-fA-F]*' || echo 'N/A'");
