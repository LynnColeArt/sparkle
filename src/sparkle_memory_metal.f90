module sparkle_memory_metal
  ! Metal-backed memory pool implementation
  ! The Sparkle Way: Unified memory done right!
  ...
  type :: metal_allocation
    ...
    integer(int64) :: size = 0             ! Size in bytes
    integer(int64) :: requested_size = 0   ! Original requested size
    logical :: in_use = .false.            ! Currently allocated?
    character(len=64) :: tag = ""          ! Debug tag
    real(real64) :: last_used = 0.0        ! For LRU eviction
...
contains
  ...
  function create_metal_pool(ctx, max_size) result(pool)
    ...
    integer(int64), intent(in), optional :: max_size
    type(metal_memory_pool) :: pool
    
    if (.not. ctx%initialized) then
      call sparkle_error_sub("Metal context not initialized", .false.)
      return
    end if
  ...
  function metal_pool_allocate(pool, size, tag) result(handle)
    ...
    if (.not. pool%initialized) then
      call sparkle_error_sub("Metal pool not initialized", .false.)
    ...
    if (slot < 0) then
      ...
      if (slot < 0) then
        call sparkle_error_sub("Metal pool full - no free slots", .false.)
        handle%is_allocated = .false.
        return
      end if
      ...
      if (.not. pool%allocations(slot)%buffer%allocated) then
        call sparkle_error_sub("Failed to create Metal buffer", .false.)
        handle%is_allocated = .false.
        return
      end if
  ...
  subroutine metal_pool_deallocate(pool, handle)
    ...
    if (slot < 1 .or. slot > pool%max_allocations) then
      call sparkle_error_sub("Invalid Metal pool slot", .false.)
      return
    end if
  ...
  subroutine metal_pool_stats(this)
    ...
    integer(int64) :: cached_size, in_use_size
    real(real32) :: cache_hit_rate
...
end module sparkle_memory_metal