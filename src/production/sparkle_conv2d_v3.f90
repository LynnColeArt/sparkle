! Sparkle Conv2D V3 - Production Integration
! ==========================================
!
...
!
! This replaces sparkle_conv2d_juggling as the main interface.

module sparkle_conv2d_v3
  use iso_fortran_env, only: real32, int32, int64, real64
  use sparkle_types
  ...
  use sparkle_conv2d_auto_selector, only: init_auto_selector
  use cpu_conv2d_adaptive, only: conv2d_adaptive
  use timing_helpers
  implicit none
  
  private
  public :: sparkle_conv2d_v3_execute
  public :: sparkle_conv2d_v3_init, sparkle_conv2d_v3_cleanup
  public :: sparkle_conv2d_v3_stats
  ...
  type :: conv2d_v3_state
    ...
    ! Statistics
    integer(int64) :: total_executions = 0
    integer(int64) :: gpu_executions = 0
    integer(int64) :: cpu_executions = 0
    real(real64) :: total_time_ms = 0.0
...
contains
  ...
  subroutine sparkle_conv2d_v3_init(cache_dir, use_dynamic, use_async, use_auto)
    ...
    print *, "🚀 Initializing Sparkle Conv2D V3 (Production)"
    print *, "============================================="
  ...
  end subroutine sparkle_conv2d_v3_init
  
  ! Main execution function
  subroutine sparkle_conv2d_v3_execute(input, weights, bias, output, &
                                      stride_h, stride_w, pad_h, pad_w, &
                                      activation, group_count)
    real(real32), intent(in) :: input(:,:,:,:)    ! [N,H,W,C]
    real(real32), intent(in) :: weights(:,:,:,:)  ! [K,KH,KW,C]
    real(real32), intent(in), optional :: bias(:)  ! [K]
    real(real32), intent(out) :: output(:,:,:,:)  ! [N,OH,OW,K]
    ...
    real(real64) :: gflops
    integer :: flops
    real(real32) :: cpu_time_ms
    
    if (.not. global_state%initialized) then
      print *, "❌ Conv2D V3 not initialized! Call sparkle_conv2d_v3_init first."
      return
    end if
    ...
    if (use_gpu) then
      ...
      if (program_id > 0) then
        ...
        block
          real(real32), allocatable :: flat_input(:), flat_weights(:), flat_output(:)
  ...
  end subroutine sparkle_conv2d_v3_execute
  
  ! Get statistics
  subroutine sparkle_conv2d_v3_stats()
    real(real64) :: avg_time_ms, avg_gflops
    real(real64) :: gpu_percentage, cpu_percentage
    ...
    print *, ""
    print *, "📊 Sparkle Conv2D V3 Statistics"
    print *, "==============================="
  ...
  end subroutine sparkle_conv2d_v3_stats
  
  ! Cleanup
  subroutine sparkle_conv2d_v3_cleanup()
  ...
  end subroutine sparkle_conv2d_v3_cleanup
  
end module sparkle_conv2d_v3