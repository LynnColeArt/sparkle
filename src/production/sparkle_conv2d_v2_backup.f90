...
contains
  ...
  subroutine conv2d_init()
    ...
    call get_environment_variable("SPARKLE_GPU_ASYNC", env_value, env_len, env_status)
    if (env_status == 0 .and. env_len > 0) then
      select case(trim(env_value))
      case("0", "off", "OFF", "false", "FALSE", "no", "NO")
        async_executor_enabled = .false.
        print *, "⚠️  GPU Async Executor disabled via SPARKLE_GPU_ASYNC environment variable"
      case default
      ...
      if (async_requested .and. selected_device == 2 .and. async_executor_enabled) then
        ...
        if (profiling_enabled) then
          if (async_requested .and. .not. async_executor_enabled) then
            print '(A)', " ℹ️  Async requested but not enabled. Set SPARKLE_GPU_ASYNC=1 to enable."
          end if
