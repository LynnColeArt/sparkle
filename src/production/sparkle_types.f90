module sparkle_types
  ...
  ! Public parameters
  integer, parameter, public :: SPARKLE_SUCCESS = 0
  integer, parameter, public :: SPARKLE_ERROR = -1
  ...
  type, public :: device_capabilities
    integer(int64) :: memory_bytes = 0
    integer(int32) :: compute_units = 0
    real(real32) :: clock_speed_ghz = 0.0
    real(real32) :: memory_bandwidth_gbps = 0.0
  ...
  type, abstract, public :: compute_device
    ...
    real(real32) :: current_load = 0.0  ! 0.0 to 1.0
  contains
  ...
  abstract interface
    ...
    subroutine deallocate_interface(self, buffer)
      import :: compute_device, sparkle_buffer
      class(compute_device), intent(inout) :: self
    ...
    function memcpy_interface(self, dst, src, size_bytes) result(status)
      import :: compute_device, sparkle_buffer, int64, int32
      class(compute_device), intent(inout) :: self
    ...
    function execute_interface(self, kernel_name, args, grid_size, block_size) result(status)
      import :: compute_device, c_ptr, int32
      class(compute_device), intent(inout) :: self
...
contains
  ...
  real(real32) function device_estimate_performance(self, flops) result(seconds)
    class(compute_device), intent(in) :: self
    integer(int64), intent(in) :: flops
    real(real32) :: tflops_per_sec
