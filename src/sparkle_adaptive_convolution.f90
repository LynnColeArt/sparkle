module sparkle_adaptive_convolution
  ! ========================================================================
  ! Adaptive Convolution Orchestrator for Sparkle
  ! 
  ...
  use sparkle_fortran_shaders_v2
...
contains
  ...
  subroutine adaptive_conv2d(input, kernel, output, profile)
    real(real32), intent(in) :: input(:,:,:,:)   ! NCHW
    real(real32), intent(in) :: kernel(:,:,:,:)  ! OCHW  
    real(real32), intent(out) :: output(:,:,:,:) ! NCHW
  ...
  subroutine profile_convolution(input, kernel, output, profile)
    ...
    real(real32), intent(in) :: kernel(:,:,:,:)
    real(real32), intent(out) :: output(:,:,:,:)
    type(convolution_profile), intent(inout) :: profile
    
    integer :: N, C, H, W, K, R, S, P, Q
    integer(int64) :: gemm_m, gemm_n, gemm_k
...
end module sparkle_adaptive_convolution