module sporkle_fused_kernels
  ! Memory wall busting through operation fusion
  ! The Sporkle Way: Make memory bandwidth irrelevant
...
contains
  ...
  subroutine fused_norm_scale_kernel(args)
    ...
    real(sp), pointer :: x(:), gamma(:), beta(:)
    integer(i64) :: i, n
    real(dp) :: mean, m2, variance
    real(sp) :: std_inv
...
end module sporkle_fused_kernels