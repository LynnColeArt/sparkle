module sporkle_kernels
  ...
  public :: sporkle_kernel, kernel_argument, kernel_builder
  ...
  type :: kernel_argument
    ...
    integer(i64), allocatable :: shape(:)
    type(memory_handle) :: data
  end type kernel_argument
  
  ! Sporkle kernel - represents pure Fortran computation
  type :: sporkle_kernel
    character(len=:), allocatable :: name
    ...
    integer(i64) :: work_items = 0  ! Total elements to process
    integer(i64) :: block_size = 256  ! For GPU execution
    
    ! Performance hints
    real(dp) :: arithmetic_intensity = 1.0_real64  ! FLOPS per byte
    logical :: is_memory_bound = .true.
    
  contains
    procedure :: validate => kernel_validate
    procedure :: estimate_flops => kernel_estimate_flops
  end type sporkle_kernel
...
contains
  ...
  function kernel_validate(this) result(is_valid)
    class(sporkle_kernel), intent(in) :: this
    logical :: is_valid
