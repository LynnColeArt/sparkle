module sporkle_benchmark
  ...
  ! The Sporkle Way: Know thy performance
  ...
  type :: benchmark_result
    ...
    real(dp) :: cold_time_ms = 0.0_real64      ! First run time
    real(dp) :: hot_time_ms = 0.0_real64       ! Best warm time
    real(dp) :: mean_time_ms = 0.0_real64      ! Average of hot runs
    real(dp) :: min_time_ms = huge(1.0_real64) ! Minimum time
    real(dp) :: max_time_ms = 0.0_real64       ! Maximum time
    real(dp) :: stddev_ms = 0.0_real64         ! Standard deviation
    real(dp) :: gflops = 0.0_real64            ! Performance in GFLOPS
    real(dp) :: bandwidth_gbs = 0.0_real64     ! Memory bandwidth
    integer(i64) :: work_items = 0
    integer :: num_runs = 0
...
contains
  ...
  function benchmark_kernel(kernel, mesh, warmup_runs, bench_runs) result(result)
    ...
    real(dp) :: total_time, total_squared
    ...
    do i = 1, n_warmup
      call sporkle_run_quiet(kernel, mesh)
    end do
  ...
  subroutine suite_run_all(this, kernels, mesh)
    class(benchmark_suite), intent(inout) :: this
    type(sporkle_kernel), intent(inout) :: kernels(:)
    type(mesh_topology), intent(inout) :: mesh
  ...
  subroutine suite_report(this)
    ...
    integer :: i
    real(dp) :: total_cold, total_hot, total_gflops
