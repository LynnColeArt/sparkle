module sporkle_cache_aware
  ! Cache-aware algorithms for breaking the memory wall
  ! The Sporkle Way: Work with the hardware, not against it
  
  use kinds
  use sporkle_config, only: sporkle_get_config, sporkle_config_type
  implicit none
  private
  ...
  integer(i64), parameter :: L1_SIZE = 32_int64 * 1024_int64         ! 32 KB
  integer(i64), parameter :: L2_SIZE = 256_int64 * 1024_int64        ! 256 KB  
  integer(i64), parameter :: L3_SIZE = 8_int64 * 1024_int64 * 1024_int64  ! 8 MB
  integer(i64), parameter :: CACHE_LINE = 64_int64                   ! 64 bytes
...
contains
  ...
  subroutine cache_aware_reduction(x, n, result, op)
    ...
    real(sp), allocatable :: partial(:)
    integer :: i, b, start_idx, end_idx
    real(sp) :: block_result
...
end module sporkle_cache_aware