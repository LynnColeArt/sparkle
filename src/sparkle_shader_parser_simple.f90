module sparkle_shader_parser_simple
      ...
      do i = 1, kernel%num_args
        if (trim(kernel%arg_names(i)) == trim(var_name)) then
          ...
          if (index(line, "integer(int32)") > 0) then
            kernel%arg_types(i) = "integer(int32)"
          else if (index(line, "real(real32)") > 0) then
            kernel%arg_types(i) = "real(real32)"
          else if (index(line, "real(real64)") > 0) then
            kernel%arg_types(i) = "real(real64)"
          end if
...
end module sparkle_shader_parser_simple