module sparkle_platform
  ! Platform detection and build configuration
  ! The Sparkle Way: Know your hardware, build accordingly
...
contains
  ...
  function detect_platform() result(info)
    ...
    call execute_command_line("uname -s > /tmp/sparkle_uname_s.txt", exitstat=status)
    
    if (status == 0) then
      ! Read the output
      open(unit=10, file="/tmp/sparkle_uname_s.txt", status='old', action='read')
      read(10, '(A)') uname_s
      close(10)
      
      ! Get kernel version too
      call execute_command_line("uname -r > /tmp/sparkle_uname_r.txt", exitstat=status)
      if (status == 0) then
        open(unit=10, file="/tmp/sparkle_uname_r.txt", status='old', action='read')
        read(10, '(A)') uname_r
        close(10)
      end if
    ...
    case(PLATFORM_LINUX)
      if (info%primary_gpu == GPU_AMD) then
        if (info%has_direct_gpu) then
          info%build_flags = trim(info%build_flags) // " -DSPARKLE_AMD_DIRECT"
        end if
        if (info%has_vulkan) then
          info%build_flags = trim(info%build_flags) // " -DSPARKLE_HAS_VULKAN"
        end if
      end if
      if (info%has_opengl) then
        info%build_flags = trim(info%build_flags) // " -DSPARKLE_HAS_OPENGL"
      end if
      
    case(PLATFORM_MACOS)
      info%build_flags = trim(info%build_flags) // &
        " -DSPARKLE_HAS_METAL -framework Metal -framework Foundation"
        
    case(PLATFORM_WINDOWS)
      info%build_flags = trim(info%build_flags) // " -DSPARKLE_HAS_DIRECTX"
    end select
  ...
  subroutine print_platform_info(info)
    ...
    print *, "🔍 Sparkle Platform Detection"
    print *, "============================"
