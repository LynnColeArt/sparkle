module sparkle_shader_parser_v2
  ! Enhanced Fortran shader parser with adaptive parameter passing support
  use iso_fortran_env
  use iso_c_binding
  use sparkle_fortran_params, only: PARAM_BUFFER_BINDING, PARAMS_UNIFORM, PARAMS_BUFFER, PARAMS_INLINE
  implicit none
  private
    ...
    if (allocated(kernel%args)) then
      do i = 1, size(kernel%args)
        ...
        if (index(line, " :: " // trim(kernel%args(i)%name)) > 0) then
          ...
          else if (index(line, "real(real32)") > 0) then
            kernel%args(i)%type_str = "real(real32)"
          else if (index(line, "real(real64)") > 0) then
            kernel%args(i)%type_str = "real(real64)"
          end if
...
end module sparkle_shader_parser_v2