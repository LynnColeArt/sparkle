module sporkle_safe_kernels
  ! Safe wrappers for kernel operations with bounds checking
  ! The Sporkle Way: Fast but safe
  
  use kinds
  use iso_c_binding, only: c_f_pointer
  use sporkle_types
  use sporkle_kernels, only: sporkle_kernel, kernel_argument, &
...
contains
  ...
  function validate_kernel_args(kernel) result(ierr)
    ...
    integer :: ierr
    integer :: i, j
    integer(i64) :: expected_size, actual_size
    ...
    if (.not. allocated(kernel%arguments)) then
      ierr = -1  ! Error
      call sporkle_error_sub("Kernel has no arguments", .false.)
      return
    end if
    ...
    do i = 1, size(kernel%arguments)
      associate(arg => kernel%arguments(i))
        ...
        if (.not. arg%data%is_allocated) then
          ierr = -1  ! Error
          call sporkle_error_sub("Argument " // trim(arg%name) // " has unallocated memory", .false.)
          return
        end if
        ...
        if (.not. allocated(arg%shape)) then
          ierr = -1  ! Error
          call sporkle_error_sub("Argument " // trim(arg%name) // " has no shape", .false.)
          return
        end if
        ...
        do j = 1, size(arg%shape)
          if (arg%shape(j) <= 0) then
            ierr = -1  ! Error
            call sporkle_error_sub("Argument " // trim(arg%name) // " has invalid shape", .false.)
            return
          end if
        ...
        case default
          ierr = -1  ! Error
          call sporkle_error_sub("Unknown type for argument " // trim(arg%name), .false.)
          return
        end select
        ...
        if (expected_size > arg%data%size) then
          ierr = -2  ! Bounds error
          call sporkle_error_sub("Memory too small for argument " // trim(arg%name), .false.)
          return
        end if
      end associate
    end do
    ...
    if (kernel%work_items <= 0) then
      ierr = -1  ! Error
      call sporkle_error_sub("Invalid work items count", .false.)
      return
    end if
  ...
  subroutine safe_kernel_wrapper(kernel)
    ...
    if (ierr /= SPORKLE_SUCCESS) then
      call sporkle_error_sub("Kernel validation failed", .false.)
      return
    end if
    ...
    block
      ...
      call get_environment_variable("SPORKLE_DEBUG", debug_env)
    ...
    if (associated(kernel%fortran_proc)) then
      ...
      call sporkle_error_sub("Kernel procedure not set", .false.)
    end if
  ...
  function create_safe_vector_add(n, x_mem, y_mem, z_mem) result(kernel)
    ...
    ! Basic validation
    if (n <= 0) then
      call sporkle_error_sub("Invalid vector size for safe_vector_add", .false.)
      return
    end if
    
    ! Check memory sizes
    if (x_mem%size < n * 4 .or. y_mem%size < n * 4 .or. z_mem%size < n * 4) then
      call sporkle_error_sub("Insufficient memory for vector operation", .false.)
      return
    end if
  ...
  subroutine safe_vector_add_impl(args)
    ...
    if (args(2)%shape(1) /= n .or. args(3)%shape(1) /= n) then
      call sporkle_error_sub("Array size mismatch in vector add", .false.)
      return
    end if
    ...
    block
      ...
      call get_environment_variable("SPORKLE_CHECK_BOUNDS", bounds_env)
      check_bounds = trim(bounds_env) == "1" .or. trim(bounds_env) == "true"
      
      if (check_bounds) then
        do i = 1, n
          if (i < 1 .or. i > n) then
            call sporkle_error_sub("Bounds violation in vector add", .false.)
            return
          end if
          z(i) = x(i) + y(i)
        end do
...
end module sporkle_safe_kernels