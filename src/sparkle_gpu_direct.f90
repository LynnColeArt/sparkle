module sparkle_gpu_direct
  ...
  type :: gpu_context
    ...
    integer(int64) :: vram_size = 0
    integer(int64) :: gart_size = 0
  ...
  interface
    ...
    function sparkle_gpu_init_c(device_path) bind(c, name="sparkle_gpu_init")
      import :: c_ptr, c_char
      character(kind=c_char), dimension(*) :: device_path
      type(c_ptr) :: sparkle_gpu_init_c
    end function
    
    ! Cleanup GPU context
    subroutine sparkle_gpu_cleanup_c(ctx) bind(c, name="sparkle_gpu_cleanup")
      import :: c_ptr
      type(c_ptr), value :: ctx
    end subroutine
    
    ! Allocate GPU memory
    function sparkle_gpu_alloc_c(ctx, size) bind(c, name="sparkle_gpu_alloc")
      import :: c_ptr, c_size_t
      type(c_ptr), value :: ctx
      integer(c_size_t), value :: size
      type(c_ptr) :: sparkle_gpu_alloc_c
    end function
    
    ! Map GPU buffer to CPU
    function sparkle_gpu_map_c(ctx, buf) bind(c, name="sparkle_gpu_map")
      import :: c_ptr
      type(c_ptr), value :: ctx, buf
      type(c_ptr) :: sparkle_gpu_map_c
    end function
    
    ! Free GPU buffer
    subroutine sparkle_gpu_free_c(ctx, buf) bind(c, name="sparkle_gpu_free")
      import :: c_ptr
      type(c_ptr), value :: ctx, buf
    end subroutine
    
    ! Test function
    subroutine sparkle_gpu_test_c() bind(c, name="sparkle_gpu_test")
    end subroutine
  end interface
...
contains
  ...
  function gpu_init(device_path) result(ctx)
    ...
    ctx%ptr = sparkle_gpu_init_c(c_path)
  ...
  subroutine gpu_cleanup(ctx)
    ...
    if (ctx%is_valid .and. c_associated(ctx%ptr)) then
      call sparkle_gpu_cleanup_c(ctx%ptr)
      ctx%ptr = c_null_ptr
      ctx%is_valid = .false.
    end if
  ...
  function gpu_allocate(ctx, size) result(buffer)
    ...
    if (ctx%is_valid) then
      buffer%ptr = sparkle_gpu_alloc_c(ctx%ptr, int(size, c_size_t))
  ...
  subroutine gpu_deallocate(ctx, buffer)
    ...
    if (ctx%is_valid .and. c_associated(buffer%ptr)) then
      ...
      call sparkle_gpu_free_c(ctx%ptr, buffer%ptr)
  ...
  function gpu_map(ctx, buffer) result(ptr)
    ...
    if (ctx%is_valid .and. c_associated(buffer%ptr) .and. .not. buffer%is_mapped) then
      ptr = sparkle_gpu_map_c(ctx%ptr, buffer%ptr)
  ...
  subroutine test_gpu_direct()
    print *, "🚀 Testing direct GPU access (The Sparkle Way)"
    call sparkle_gpu_test_c()
  end subroutine test_gpu_direct

end module sparkle_gpu_direct