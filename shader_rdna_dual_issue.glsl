...
void main() {
  ...
  // Convolution with dual-issue FMA
  for (int c = 0; c < params.C; c += 2) {
    for (int kh = 0; kh < params.kernel_size; kh++) {
      for (int kw = 0; kw < params.kernel_size; kw++) {
        ...
        if (h_in >= 0 && h_in < params.H && w_in >= 0 && w_in < params.W) {
          // Process two channels for dual-issue
          if (c < params.C) {
          ...
          if (c + 1 < params.C) {
            int in_idx2 = ((n * params.C + c + 1) * params.H + h_in) * params.W + w_in;
            int weight_idx2 = ((k * params.C + c + 1) * params.kernel_size + kh) * params.kernel_size + kw;
