program test_pm4_summit_kernel
  ...
  integer, parameter :: BATCH = 1_i32
  integer, parameter :: HEIGHT = 256_i32, WIDTH = 256_i32
  integer, parameter :: IN_CHANNELS = 64_i32, OUT_CHANNELS = 64_i32
  integer, parameter :: KERNEL_H = 3_i32, KERNEL_W = 3_i32
  integer, parameter :: KO_PARALLEL = 4_i32  ! Process 4 output channels in parallel
  
  ! Memory layout sizes
  integer(i64), parameter :: INPUT_SIZE = int(BATCH, i64) * int(HEIGHT, i64) * int(WIDTH, i64) * int(IN_CHANNELS, i64) * 4_i64
  integer(i64), parameter :: KERNEL_SIZE = int(OUT_CHANNELS, i64) * int(KERNEL_H, i64) * int(KERNEL_W, i64) * int(IN_CHANNELS, i64) * 4_i64
  integer(i64), parameter :: OUTPUT_SIZE = int(BATCH, i64) * int(HEIGHT, i64) * int(WIDTH, i64) * int(OUT_CHANNELS, i64) * 4_i64
  ...
  integer :: status, i, iter
  real(dp) :: gflops, best_gflops = 0.0_dp
  integer(i64) :: gpu_cycles, best_cycles = huge(1_i64)
  ...
  if (.not. (input_buffer%is_valid() .and. kernel_buffer%is_valid() .and. &
             output_buffer%is_valid() .and. timer_buffer%is_valid() .and. &
  ...
  total_flops = int(BATCH, i64) * int(HEIGHT, i64) * int(WIDTH, i64) * &
                int(OUT_CHANNELS, i64) * int(KERNEL_H, i64) * int(KERNEL_W, i64) * &
                int(IN_CHANNELS, i64) * 2_i64  ! 2 ops per multiply-accumulate
  ...
  do iter = 1, 10
    ...
    ! Submit CP alive test (Mini's EOP validation: ~17 dwords)
    ! Note: Using timer_buffer in BO list as Mini recommended
    status = sp_submit_ib_with_bo(ctx_ptr, ib_buffer%bo_ptr, 17, &
    ...
    print '(A,Z16,A,Z16)', "  EOP timestamps: T0=0x", timer_data(1), " T1=0x", timer_data(2)
    
    ! Check if EOP timestamps are working
    if (timer_data(1) > 0 .and. timer_data(2) > timer_data(1)) then
      print *, "  ✅ Mini's EOP timestamps working!"
      gpu_cycles = timer_data(2) - timer_data(1)
    else
      print *, "  ❌ EOP timestamps still zero/invalid"
      gpu_cycles = 0
    end if
    
    if (gpu_cycles > 0) then
      ! Assume 2.5 GHz GPU clock for timing calculation
      gflops = real(total_flops, dp) / (real(gpu_cycles, dp) / 2.5d9) / 1.0d9
  ...
  if (best_cycles < huge(1_i64)) then
    ...
    print '(A,F6.2,A)', "  GPU execution time: ", real(best_cycles, dp) / 2.5d9 * 1000.0d0, " ms"
  else
...
contains
  ...
  subroutine build_summit_compute_ib(ib_data, input_va, kernel_va, output_va, timer_va)
    ...
    ib_data(idx) = ior(ishft(3_i32, 30_i32), ior(ishft(5_i32, 16_i32), PM4_RELEASE_MEM))  ! Type 3, 6 dwords
    idx = idx + 1
    ...
    print '(A,I0,A)', "Built PM4 IB with ", idx-1, " dwords (Mini's CP alive test)"
    
    ! Ensure we don't exceed IB size
    if (idx > size(ib_data)) then
      print *, "ERROR: IB overflow!"
    end if
  end subroutine

end program test_pm4_summit_kernel